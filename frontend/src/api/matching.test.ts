...
describe('matchingApi', () => {
  ...
  it.each([
    ['신규', 201],
    ['멱등 재요청', 201],
  ] as const)('%s 차단 HTTP %s를 성공으로 처리한다', async (_case, status) => {
    const response = {
      blockId: 1,
      blockedMemberId: 27,
    ...
    await expect(matchingApi.submitBlock(30, { blockedMemberId: 27 }))
      .resolves.toEqual(response);

    expect(fetchMock).toHaveBeenCalledWith(
      '/api/match-groups/30/blocks',
      expect.objectContaining({
        credentials: 'include',
        method: 'POST',
        body: JSON.stringify({ blockedMemberId: 27 }),
      }),
    );
    expect(Object.keys(JSON.parse(fetchMock.mock.calls[0][1].body)))
      .toEqual(['blockedMemberId']);
    expect(JSON.parse(fetchMock.mock.calls[0][1].body))
      .not.toHaveProperty('blockerMemberId');
  });
});
