...
/**
 * 제재 사유·기간 안내 card(docs/19 4.8).
 *
 * 두 곳에서 쓴다. 영구정지는 로그인 화면(로그인 자체가 막히므로), 이용정지는 활동을 시도해
 * 403을 받은 순간의 dialog(로그인 상태로 조회는 계속하므로)다.
 *
 * 사유 문구는 서버가 만든 reasonMessage를 그대로 쓴다. 화면에서 사유 code를 문구로 바꾸면
 * 신고자 보호 심사를 두 곳에서 해야 한다. 문구 정의는 backend의 `MemberSanctionReason` 한 곳이다.
 */
export default function AccountRestrictionNotice({ notice, prominent = false }: Props) {
  const permanent = notice.status === 'BANNED';
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <h2 id="account-restriction-title" className="text-[15px] font-bold text-ink">
          {permanent ? '영구정지된 계정이에요' : '이용정지 중이에요'}
        </h2>
      </div>

      <dl className="mt-4 flex flex-col gap-3">
        <div>
          <dt className="text-[13px] text-ink/55">제한 사유</dt>
          <dd className="mt-0.5 text-[14px] font-semibold text-ink">{notice.reasonMessage}</dd>
        </div>
        <div>
          <dt className="text-[13px] text-ink/55">제한 기간</dt>
          <dd className="mt-0.5 text-[14px] font-semibold text-ink">
            {permanent ? '기간 제한 없음' : `${formatSeoulDateTime(notice.suspendedUntil)}까지`}
          </dd>
        </div>
      </dl>
