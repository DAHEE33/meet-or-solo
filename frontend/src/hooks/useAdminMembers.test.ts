...
describe('createAdminMembersSession', () => {
  it('검색 변경은 cursor를 초기화하고 늦은 목록 응답을 무시한다', async () => {
    ...
    const session = createAdminMembersSession({ list, detail: vi.fn(), act: vi.fn(), forceWithdraw: vi.fn(), adjustMannerTemperature: vi.fn() }, (next) => { state = next; }); void session.load(); void session.applyFilters({ query: '새회원', status: '', role: 'USER' }); second.resolve(page([detail(2)])); await Promise.resolve(); first.resolve(page([detail(1)])); await Promise.resolve();
  ...
  it('이중 제출은 한 번만 호출하고 실패 전 snapshot을 유지한다', async () => {
    ...
    const session = createAdminMembersSession({ list: vi.fn().mockResolvedValue(page([detail(1), detail(2)])), detail: vi.fn().mockResolvedValue(detail(1)), act, forceWithdraw: vi.fn(), adjustMannerTemperature: vi.fn() }, (next) => { state = next; }); await session.load(); await session.openDetail(1);
  ...
  it('실패하면 상세와 확인 요청을 유지한다', async () => {
    let state!: AdminMembersState; const session = createAdminMembersSession({ list: vi.fn().mockResolvedValue(page()), detail: vi.fn().mockResolvedValue(detail(1)), act: vi.fn().mockRejectedValue(new Error('fail')), forceWithdraw: vi.fn(), adjustMannerTemperature: vi.fn() }, (next) => { state = next; }); await session.load(); await session.openDetail(1);
