...
const detail = (id: number, status: AdminMemberDetail['status'] = 'ACTIVE'): AdminMemberDetail => ({ memberId: id, nickname: `회원${id}`, profileImageUrl: null, role: 'USER', status, penaltyScore: 0, mannerTemperature: 36.5, suspendedAt: null, suspendedUntil: null, createdAt: '2026-08-16T09:00:00+09:00', lastLoginAt: null, testAccount: false, recentValidReportCount: 0, safetyReviewRequired: false, reports: [], actions: [], mannerTemperatureAdjustments: [] });
...
describe('createAdminMembersSession', () => {
  ...
  it('이중 제출은 한 번만 호출하고 실패 전 snapshot을 유지한다', async () => {
    ...
    const session = createAdminMembersSession({ list: vi.fn().mockResolvedValue(page([detail(1), detail(2)])), detail: vi.fn().mockResolvedValue(detail(1)), act, forceWithdraw: vi.fn(), adjustMannerTemperature: vi.fn(), updateTestAccount: vi.fn() }, (next) => { state = next; }); await session.load(); await session.openDetail(1);
  ...
  it('실패하면 상세와 확인 요청을 유지한다', async () => {
    let state!: AdminMembersState; const session = createAdminMembersSession({ list: vi.fn().mockResolvedValue(page()), detail: vi.fn().mockResolvedValue(detail(1)), act: vi.fn().mockRejectedValue(new Error('fail')), forceWithdraw: vi.fn(), adjustMannerTemperature: vi.fn(), updateTestAccount: vi.fn() }, (next) => { state = next; }); await session.load(); await session.openDetail(1);
