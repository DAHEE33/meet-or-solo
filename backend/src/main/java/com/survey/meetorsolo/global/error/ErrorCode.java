...
public enum ErrorCode {
    ...
    /**
     * 관리자 ID/PW 로그인 실패(docs/30). 아이디 오류·비밀번호 오류·잠금·권한 없음을 구분하지
     * 않고 전부 이 code로 응답한다. 구분해서 알려주면 아이디 목록을 열거할 수 있다.
     */
    ADMIN_LOGIN_FAILED(HttpStatus.UNAUTHORIZED, "ADMIN_LOGIN_FAILED",
    ...
    ADMIN_MEMBER_MANNER_TEMPERATURE_CONFLICT(HttpStatus.CONFLICT, "ADMIN_MEMBER_MANNER_TEMPERATURE_CONFLICT", "매너온도가 조회 시점과 달라졌습니다. 다시 확인해 주세요."),
    ...
    ADMIN_SAFETY_ALERT_INVALID_REQUEST(HttpStatus.BAD_REQUEST, "ADMIN_SAFETY_ALERT_INVALID_REQUEST", "관리자 안전 알림 요청 값이 올바르지 않습니다."),
    ADMIN_SAFETY_ALERT_NOT_FOUND(HttpStatus.NOT_FOUND, "ADMIN_SAFETY_ALERT_NOT_FOUND", "안전 알림 정보를 찾을 수 없습니다."),
    ADMIN_SAFETY_ALERT_STATUS_CONFLICT(HttpStatus.CONFLICT, "ADMIN_SAFETY_ALERT_STATUS_CONFLICT", "현재 알림 상태에서는 요청한 변경을 처리할 수 없습니다."),
    ...
    CONTENT_TARGET_NOT_FOUND(HttpStatus.NOT_FOUND, "CONTENT_TARGET_NOT_FOUND", "축제 또는 관광지 정보를 찾을 수 없습니다."),
    CONTENT_COMMENT_INVALID_REQUEST(HttpStatus.BAD_REQUEST, "CONTENT_COMMENT_INVALID_REQUEST", "댓글 요청이 올바르지 않습니다."),
    CONTENT_COMMENT_NOT_FOUND(HttpStatus.NOT_FOUND, "CONTENT_COMMENT_NOT_FOUND", "댓글을 찾을 수 없습니다."),
    CONTENT_COMMENT_FORBIDDEN(HttpStatus.FORBIDDEN, "CONTENT_COMMENT_FORBIDDEN", "본인이 작성한 댓글만 삭제할 수 있습니다."),
    CONTENT_COMMENT_PROFILE_REQUIRED(HttpStatus.FORBIDDEN, "CONTENT_COMMENT_PROFILE_REQUIRED", "닉네임을 설정한 뒤 댓글을 남길 수 있습니다."),
    CONTENT_COMMENT_TOO_FREQUENT(HttpStatus.TOO_MANY_REQUESTS, "CONTENT_COMMENT_TOO_FREQUENT", "댓글은 잠시 후 다시 남길 수 있습니다."),
    CONTENT_COMMENT_CHECKIN_REQUIRED(HttpStatus.FORBIDDEN, "CONTENT_COMMENT_CHECKIN_REQUIRED", "이 축제에 체크인한 뒤 댓글을 남길 수 있습니다."),
    INQUIRY_INVALID_REQUEST(HttpStatus.BAD_REQUEST, "INQUIRY_INVALID_REQUEST", "문의 요청이 올바르지 않습니다."),
    INQUIRY_NOT_FOUND(HttpStatus.NOT_FOUND, "INQUIRY_NOT_FOUND", "문의 정보를 찾을 수 없습니다."),
    INQUIRY_FORBIDDEN(HttpStatus.FORBIDDEN, "INQUIRY_FORBIDDEN", "본인이 등록한 문의만 확인할 수 있습니다."),
    INQUIRY_CLOSED(HttpStatus.CONFLICT, "INQUIRY_CLOSED", "종결된 문의입니다. 새 문의로 등록해 주세요."),
    INQUIRY_TOO_MANY_OPEN(HttpStatus.TOO_MANY_REQUESTS, "INQUIRY_TOO_MANY_OPEN", "답변을 기다리는 문의가 많습니다. 답변을 받은 뒤 다시 등록해 주세요."),
    ADMIN_INQUIRY_INVALID_REQUEST(HttpStatus.BAD_REQUEST, "ADMIN_INQUIRY_INVALID_REQUEST", "관리자 문의 요청 값이 올바르지 않습니다."),
    ADMIN_INQUIRY_NOT_FOUND(HttpStatus.NOT_FOUND, "ADMIN_INQUIRY_NOT_FOUND", "문의 정보를 찾을 수 없습니다."),
    ADMIN_INQUIRY_STATUS_CONFLICT(HttpStatus.CONFLICT, "ADMIN_INQUIRY_STATUS_CONFLICT", "현재 문의 상태에서는 요청한 변경을 처리할 수 없습니다."),
