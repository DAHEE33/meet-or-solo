...
/**
 * 관리자 문의 목록 cursor codec.
 *
 * <p>{@code AdminSafetyAlertCursorCodec}·{@code AdminMemberCursorCodec}과 같이 관리자 전용
 * HMAC Secret을 공유하고 payload prefix로 도메인을 분리한다. 새 환경변수를 요구하지 않고,
 * 다른 도메인의 cursor를 이 목록에 그대로 넣을 수 없다. JWT Secret과는 분리된 키다.
 */
@Component
public class AdminInquiryCursorCodec {

    private static final String HMAC_SHA256 = "HmacSHA256";
    private static final String PAYLOAD_PREFIX = "inquiry:v1:";
    private static final int MINIMUM_SECRET_BYTES = 32;

    private final byte[] secret;

    public AdminInquiryCursorCodec(
    ...
    public String encode(OffsetDateTime createdAt, long inquiryId, String fingerprint) {
        String payload = PAYLOAD_PREFIX + createdAt.toInstant().getEpochSecond() + ":"
                + createdAt.getNano() + ":" + inquiryId + ":" + fingerprint;
    ...
    public Cursor decode(String value, String expectedFingerprint) {
        try {
            ...
            // fingerprint가 다르면 filter가 바뀐 것이다. 그대로 이어 읽으면 페이지 경계에서
            // 항목이 중복·누락되므로 거절한다.
            ...
            long inquiryId = Long.parseLong(values[2]);
            if (nano < 0 || nano > 999_999_999 || inquiryId <= 0) {
                throw invalidCursor();
            }
            return new Cursor(
    ...
    private byte[] sign(String payload) {
        try {
            ...
            throw new IllegalStateException("관리자 문의 cursor 서명에 실패했습니다.", exception);
        }
    }
    ...
    private BusinessException invalidCursor() {
        return new BusinessException(
                ErrorCode.ADMIN_INQUIRY_INVALID_REQUEST, "cursor 값이 올바르지 않습니다.");
    }

    public record Cursor(OffsetDateTime createdAt, long inquiryId) {
    }
}
