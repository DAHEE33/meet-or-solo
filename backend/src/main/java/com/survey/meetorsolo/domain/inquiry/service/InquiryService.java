 ...
 * 이의제기는 고객센터 이메일 안내를 유지한다(docs/28_MEMBER_INQUIRY_DESIGN.md 2.1).
 *
 * <p>정지({@code SUSPENDED}) 회원은 등록할 수 있어야 한다. 제재 사유를 다툴 수 없으면 제재가
 * 일방적이 되므로 {@code SuspendedActivityPolicy}의 허용 목록에 등재했다(docs/28 2.3).
 *
...
public class InquiryService {
     ...
     * (docs/28 5.1, {@code ContentCommentService.MIN_COMMENT_INTERVAL}과 같은 판단).
     */
     ...
     * 판정은 {@code Inquiry.hasUnreadAnswer()}와 같은 식이어야 한다(docs/28 4.2).
     */
    @Transactional(readOnly = true)
     ...
     * 목록에 다시 뜨지 않아 재질문이 묻힌다(docs/28 5.4).
     */
    @Transactional
