...
dev nginx/docker-compose 템플릿은 7단계에서 추가했지만, `docker-compose.prod.yml`, prod nginx 설정, GitHub Actions는 아직 생성하지 않습니다.
...
- `nginx`: `frontend/dist` 정적 파일 서빙, `/api` 요청을 `backend:8080`으로 reverse proxy, 외부 `80` 포트만 publish
