...
| `MatchProposalResponseServiceIntegrationTest` | 36 | PostgreSQL | round 1/2 응답, 최소 인원 확정, pool 정책, 멱등성, 동시성, rollback |
...
3. matching 신청·proposal REST API
4. application-level `POOL_ENTRY` 실행 경로
5. frontend proposal UI와 인원 미달 modal
6. WebSocket STOMP 상태 동기화
