...
| attempt lifecycle | `WAITING_RESPONSES` 생성까지만 구현 |
| proposal lifecycle | `INITIAL_MATCH`, round 1, `SENT` 생성까지만 구현 |
| cooldown | active cooldown 제외 조회만 구현. penalty/cooldown 생성은 미구현 |
| embedding | V11 schema만 존재. matching score에는 사용하지 않음 |
| response/group | V3 테이블은 존재하지만 matching entity/service가 없음 |

### 미구현 또는 후속 계획

- proposal 수락·거절·timeout 처리
- `match_responses` 생성
- penalty/cooldown 생성
- 인원 미달 재확인과 `allowMinimumTwo`
- 최종 `match_groups`, `match_group_members` 생성
...
stateDiagram-v2
    ...
    PROPOSED --> MATCHED: "schema only / 미구현"
    WAITING --> CANCELLED: "schema only / 미구현"
    WAITING --> COOLDOWN: "schema only / 미구현"
```

`MATCHED`, `CANCELLED`, `COOLDOWN`은 V3가 허용하지만 현재 matching 운영 코드에서는 전이시키지 않습니다.
...
| --- | --- |
| proposal 수락·거절 | 미구현 |
| proposal timeout 상태 처리 | schema/fixture만 존재, 운영 처리 미구현 |
| `match_responses` 생성 | schema만 존재 |
| penalty/cooldown 생성 | schema만 존재 |
| 인원 미달 재확인 | 정책·V10 schema만 존재 |
| `allowMinimumTwo` 적용 | field와 정책만 존재, 최초 조합에서는 의도적으로 미사용 |
| 최종 group/member 생성 | V3 schema만 존재 |
...
1. proposal 수락·거절과 `match_responses`
2. proposal timeout과 penalty/cooldown
3. 인원 미달 재확인과 `allowMinimumTwo`
4. 최종 `match_groups`, `match_group_members` 생성
5. matching 신청·proposal REST API
6. application-level `POOL_ENTRY` 실행 경로
7. frontend proposal UI
8. WebSocket STOMP 상태 동기화
