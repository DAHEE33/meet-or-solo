...
체크인과 확정 매칭의 유효시간은 MVP 기준 각각 1시간입니다. 정상 완료는
유효 참여자 전원 도착 시 자동 처리하지만, 완료 직후 새로운 매칭을 허용하지
않고 기존 group의 `confirmed_at + 1시간`까지 재매칭을 제한합니다. 제한 종료
뒤에는 현장 체크인을 다시 통과한 후 새 매칭을 신청합니다.

Backend는 완료 group/member 이력에서 이 제한을 계산하며 penalty cooldown과
분리합니다. Frontend는 완료 제한 응답을 기준으로 취소 화면과 다른 완료 전용
card를 복원합니다.
...
fallback에 활용하고, 카페·편의점·주차장처럼 식별 가능한 실제 장소 후보는
Kakao Local API의 키워드·카테고리 검색으로 보완합니다. 운영자는 축제별로
안전하고 찾기 쉬운 만남 장소를 여러 개 검토·등록합니다. 매칭 그룹이 확정될
때 등록된 장소 중 하나를 배정하고 `match_groups`에 snapshot으로 저장하므로,
같은 그룹은 항상 같은 핀을 보지만 서로 다른 그룹에는 다른 장소가 배정될 수
있습니다. MVP는 순환 배정으로 시작하고 이후 같은 시간대 배정 수를 고려한
혼잡도 분산 방식으로 확장합니다.
