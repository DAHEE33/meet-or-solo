...
### 만남 수명주기 — 자동 하네스 + 수동 시나리오

매칭 확정부터 도착·이탈·종료·보상·신고까지는 두 겹으로 관리합니다.

| 층 | 무엇 | 덮는 것 |
| --- | --- | --- |
| 자동 | `MatchMeetingLifecycleHarnessTest` | 실제 PostgreSQL 위에서 HTTP 경로로 컨트롤러 → 서비스 → 배치 → 보상 이벤트 → 재매칭 제한까지 **연결이 이어지는지** |
| 수동 | [`docs/30_MATCHING_MANUAL_TEST_SCENARIOS.md`](30_MATCHING_MANUAL_TEST_SCENARIOS.md) | 화면 문구·버튼 노출, 환경 설정이 실제 값으로 도는지, 스케줄러가 실제로 집어가는지 |

개별 서비스 테스트가 규칙을 하나씩 고정하는 것과 목적이 다릅니다. 규칙이 다 맞아도 그 사이가
끊기면 사용자에게는 아무것도 동작하지 않으므로, **연결 자체를 검증하는 테스트를 따로 둡니다.**
