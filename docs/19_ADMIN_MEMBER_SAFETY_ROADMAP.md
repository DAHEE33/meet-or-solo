...
##### 전문 확인 전 체크 잠금 (후속 브랜치 `feature/wbs-10-b-consent-view-gate`)

원문을 붙인 것만으로는 **보지 않고 체크할 수 있었다.** 전문을 열기 전에는 체크박스를 잠근다.

- 약관과 처리방침을 각각 따로 판정한다. 하나를 봤다고 둘 다 풀리지 않는다.
- **잠금만 풀고 체크는 사용자가 직접 누른다.** 모달의 `확인했어요`가 체크까지 켜면 "확인"을
  "동의"로 간주하는 셈이 되어 동의 여부를 다툴 때 방어가 약해진다.
- **"봤다"의 기준은 모달을 연 시점이다.** 닫는 시점으로 잡으면 닫기 경로(`확인했어요`/`X`)
  하나가 빠져도 영구히 잠긴 체크박스가 되고 가입 자체가 막힌다.
- 잠긴 이유와 풀렸다는 사실을 `ConsentCheckbox`의 `hint`로 같은 자리에서 알린다.
- 전문 미확인 상태로 완료를 누르면 "동의해 주세요"가 아니라 "전문을 먼저 확인해 주세요"다.
