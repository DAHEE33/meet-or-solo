...
- 상태: `IN_PROGRESS` — 4.1 관리자 신고 검토 완료, 4.2 관리자 회원 조회·제재 완료,
  ...
  4.11 만남 성립 판정 정리 구현 완료(수동 검증 대기) — 신고 대상을 도착자가 있는 매칭으로 좁히고,
  완료 판정을 도착 시점에서 확정 + 1시간으로 옮겼다.
  **남은 항목은 4.7의 나머지(임베딩 재시도, 외부 호출 transaction 분리, 재동의 인프라),
  4.9의 PR C(30도 매칭 제한)·D(후기), 4.11.5의 회원 알림, 그리고 2절 표의 기본 통계다.**
...
### 4.7 동의·개인정보 후속 — 진행 중 (동의 원문 노출 완료)
...
#### 4.7.1 동의 시점 원문 노출 — 완료

브랜치: `feature/wbs-10-b-consent-followup`

상태: Frontend 구현·자동 테스트 완료. 브라우저 수동 검증 대기

가입 화면의 필수 동의 2종(`TERMS`, `PRIVACY`)에 **볼 것이 없던 문제**를 해결했습니다.
AI 관련 동의 2종은 `AiConsentSection`이 `자세히`로 목적·항목·보관기간·거부 시 불이익을
펼쳐 보여주고 있었는데, 정작 법정 필수 동의 2종은 체크박스 라벨 한 줄뿐이었습니다.

- `legalDocuments.ts`에 이용약관 15개 조와 개인정보처리방침 14개 절의 **원문**을 두고,
  체크박스 옆 `전문 보기`가 `LegalDocumentModal`로 띄운다. 요약은 `consentNotice.ts`의
  `TERMS_NOTICE`·`PRIVACY_NOTICE`가 기존 `자세히` 형식으로 보여준다.
- **페이지 이동이 아니라 모달이다.** 이 문서를 여는 곳은 가입 마지막 단계이고 그 화면에는
  닉네임·성별·연령대·여행 스타일·취향 글이 이미 입력돼 있다. 다른 페이지로 보내면 입력이
  전부 사라진다.
- `AiConsentSection` 안에 있던 `ConsentCheckbox`를 파일로 분리해 약관 동의에서도 쓴다.
  마크업은 옮기기 전과 같고 AI 동의 화면의 동작은 바뀌지 않는다.
- **동의 저장 흐름은 손대지 않았다.** `agreeAll(SIGNUP_CONSENT_TYPES)` 호출 순서와
  체크 상태 판정은 그대로다. 보기 버튼만 늘었다.
- 원문 내용은 **실제 구현에서 뽑았다.** 없는 기능을 약속하지 않고 하는 일을 빠뜨리지도
  않았다. 대표적으로 체크인은 좌표를 전송받아 축제와의 거리만 계산하고 좌표를 저장하지
  않으므로(`festival_checkins`에 `distance_meters`만 있다) 처리방침에 그대로 적었다.
- 문의처는 backend의 `SUPPORT_CONTACT_EMAIL`과 같은 값을 `VITE_SUPPORT_CONTACT_EMAIL`로
  주입한다. **값이 비면 문구를 조용히 지우지 않고 설정되지 않았다는 사실을 화면에 남긴다.**
  제재 안내에서 환경변수 누락이 문구를 소리 없이 없앴던 경로와 같은 실수를 막기 위한 것이다.

##### 전문을 확인해야 동의되는 흐름 (브랜치 `feature/wbs-10-b-consent-view-gate`)

원문을 붙인 것만으로는 **보지 않고 체크할 수 있었다.** 전문을 확인해야 동의가 되게 한다.

처음에는 체크박스를 잠그고 안내 문구를 다는 방식(PR #69)이었으나, 사용자 확인을 거쳐
**누르면 전문이 바로 뜨는 방식**으로 조정했다.

- 체크박스는 잠그지 않는다. 전문을 안 본 상태에서 누르면 체크되는 대신 전문이 뜨고,
  모달의 `확인했고 동의합니다`를 누르면 그때 체크된다.
- `X`로 닫으면 동의하지 않는다. 이미 열람했으므로 다음 클릭부터는 바로 체크된다.
- **`전문 보기` 버튼으로 연 경우는 동의로 잇지 않는다.** 열람 의사일 뿐이므로 버튼 문구도
  `확인했어요`로 다르고 확인해도 체크되지 않는다.
- 확인을 동의로 보는 근거는 **사용자가 먼저 "동의합니다"를 누른 뒤 전문을 본 흐름**이라는
  점이다. 동의 의사가 열람보다 앞선다.
- **"봤다"의 기준은 모달을 연 시점이다.** 닫는 시점으로 잡으면 닫기 경로(확인 버튼/`X`)
  하나가 빠져도 영원히 체크되지 않는 체크박스가 되고 가입 자체가 막힌다.
- 약관과 처리방침을 각각 따로 판정한다. 하나를 봤다고 둘 다 열람 처리되지 않는다.
- AI 동의 2종은 선택 동의이고 전문이 따로 없어 적용하지 않는다.

##### 공개 운영 전 운영자가 확정해야 하는 값 4건

원문에 넣을 수 없어 `legalDocuments.ts` 상단 `TODO(운영)`으로 남겼습니다. 코드에서 알 수
없거나 운영 정책 결정이 필요한 항목입니다.

1. 개인정보 보호책임자의 성명·직책 (현재 "서비스 운영자"로만 표기)
2. 신고·제재 기록의 구체적 보유 기간 (현재 "안전 목적 보관"으로만 표기)
3. 프로필 이미지를 저장하는 오브젝트 스토리지 사업자명 (환경설정 값이라 코드에서 알 수 없다)
4. 만 14세 이상 가입 제한 유지 여부 (현재 코드에는 연령 확인 절차가 없다)

또한 이 원문은 **법률 검토를 받지 않은 초안**입니다. 공개 운영 전에 검토가 필요합니다.

#### 4.7.2 동의 조회 범위 확장 — 하지 않기로 결정

`GET /api/members/me/consents`를 AI 2종에서 `API_MANAGED` 4종으로 넓히는 안을 검토했고
**보류했습니다.** 4.7 목록의 해당 항목은 문제 지적이 아니라 현황 서술이었습니다.

- 프론트의 소비처는 `hasAllAiConsents` 하나뿐이고 그것은 `AI_CONSENT_TYPES`만 본다.
  `TERMS`·`PRIVACY`를 응답에 실어도 **읽는 곳이 없다.**
- 값이 생기는 시점은 마이페이지에 **동의 내역 화면**(항목·동의 일시·원문 보기)이 생길
  때다. 화면 없이 API만 넓히면 다음 사람이 "왜 4종인가"를 다시 조사한다.
- 동의의 법적 근거는 **동의 시점에 원문을 보여주는 것**(4.7.1)으로 충족된다. 사후 조회는
  편의 기능이다. 필요해지면 화면과 함께 한 브랜치에서 진행한다.

#### 4.7.3 남은 항목

| 항목 | 상태 |
| --- | --- |
| 임베딩 재시도 경로 부재 | 미착수. 재시도 대상 실패 사유를 `TIMEOUT`·`CONNECT_FAILED`·`RATE_LIMITED`·`UPSTREAM_ERROR`로 좁히는 판단이 선행 |
| 외부 호출과 transaction 분리 | 미착수. 저장 순서를 바꾸는 작업이라 취향 원문 롤백 버그(`V32` 작업에서 고친 것)를 되살릴 위험이 있어 테스트로 계약을 고정한 뒤 진행 |
| 동의 버전 무시 / 약관 소급 동의 | 미착수. 조회 조건만 바꾸면 **기존 동의자 전원이 취향을 잃는다.** 재동의 게이트와 마이그레이션 방식을 함께 설계해야 한다 |
| `PROFILE_REQUIRED` 상태의 프로필 수정 | 미결. 정상 흐름에서 도달 불가한 엣지 케이스로 확인됐다. 백엔드 게이트는 법적 요구를 지키는 자리라 완화하지 않는다 |
...
**만남 완료 보상**을 먼저 넣었다. 후기보다 나은 점이 셋이다.
...
댓글 작성량을 온도에 반영하자는 안은 **채택하지 않았다.** 매너온도는 "만남에서의 매너"
지표인데 댓글은 만남과 무관하고, 혼자 무한히 쓸 수 있어 조작이 너무 쉽다. 반영하면 "매너
좋은 사람"이 아니라 "글 많이 쓴 사람"의 온도가 높아진다. 콘텐츠 활동 장려가 목적이라면
별도 지표로 다룬다.
...
구현 결과와 판단 근거는 `docs/10_PROGRESS_LOG.md`의
`[10-B] 매너온도 상승 경로와 회원 노출 (docs/19 4.9 · PR B)`을 따른다.

| 경로 | 값 | 상한 | 비고 |
| --- | --- | --- | --- |
| 시작 | `36.50` | — | |
| 신고 유효 판정 | `-2.00` | 하한 `20.00` | **`5.00`에서 낮춤** |
| 만남 완료 | `+0.50` | `42.00` | 그룹당 1회. 후기와 무관 |
| 시간 경과 | `30일마다 +0.50` | **`36.50`** | 활동 기반 상승과 상한이 다르다 |
| 관리자 수동 | 자유 | `20.00`~`42.00` | PR A |
| 노쇼 | **변화 없음** | — | `penalty_score`만 오른다. 현행 유지 |

**시간 경과 회복의 상한만 시작값이다.** 상한(`42.00`)까지 올리면 아무 활동도 하지 않은
회원이 가만히 있다가 상한에 도달해, 지표가 "가입한 지 얼마나 됐나"를 뜻하게 된다. 시작값을
넘는 구간은 실제로 만남을 마쳐야 오른다.

**`+1.00`이 아니라 `+0.50`이다.** `+1.00`이면 완료 6번에 상한을 찍어 열심히 참여한 회원이
전부 `42.00`에 몰리고 관리자가 지표를 봐도 구분이 안 된다.

#### 시간 경과 회복은 선택이 아니라 필수다 — 데드락

PR C에서 30도 제한을 넣으면 **만남 기반 상승 경로만으로는 원리적 데드락**이 생긴다.

```
신고 4건 → 28.5도 → 매칭 금지 → 만남 불가 → 완료·후기 불가 → 회복 불가 → 영구 배제
```

후기도 마찬가지다. 만남이 있어야 후기를 받는다. 탈출구는 셋뿐이고 그중 둘을 PR A·B에서
확보했다.

| 탈출구 | 상태 |
| --- | --- |
| 관리자 수동 조정 | PR A 완료. 사람 손에 의존해 운영 부담이 크다 |
| 시간 경과 회복 | **PR B 완료.** 자동 탈출구 |
| 제한을 매칭 금지가 아니라 알림으로만 | 제한 자체를 포기 |

#### PR C 착수 전 반드시 볼 것 — 왜 차감량을 낮췄나
...
**확정: 차감량을 `5.00` → `2.00`으로 낮췄다(PR B에서 적용 완료).** 그러면 30도 아래가
되려면 신고 4건(`36.5 - 8 = 28.5`)이 필요해 알림(3건)이 먼저 뜨고 그다음 제한이 걸린다.

원래는 "제한과 같은 PR에서 함께 바꾼다"고 정했지만, PR B에서 **만남 완료 보상이 함께
들어가면서** 하강폭만 낮아져 제재가 약해지는 상황이 아니게 됐다. 신고 1건(`-2.00`)은
완료 4번(`+2.00`)으로 정확히 회복되며, 이 관계를
`MemberMannerTemperatureRiseTest.신고_1건은_완료_4번으로_정확히_회복된다`가 고정한다.
...
매너온도는 지금까지 관리자만 보는 내부 지표였다. 회원 화면과 회원용 API 어디에도 없었다.
PR B에서 **본인 것만** 노출한다.

| 위치 | 형태 |
| --- | --- |
| 마이페이지 | 게이지 + 숫자 + 구간별 안내 문구 |
| 매칭 화면 | 우상단 한 줄(compact) |

**다른 회원의 온도는 어디에도 노출하지 않는다.** 낮은 온도는 "신고를 받은 적이 있다"를
그대로 드러내고, 같은 만남에 있던 사람이 보면 누가 신고했는지 좁힐 수 있다. `docs/19` 4.8의
신고자 보호 원칙과 같은 논리다.

**`penalty_score`는 계속 노출하지 않는다.** 노쇼 누적으로 쿨타임을 거는 내부 운영 값이다.
`MatchingRestrictionResponse`에는 예전부터 담겨 있었으나 화면이 그린 적이 없고, PR B에서도
그리지 않는다.
...
| 온도 반영 | 완료 보상(`+0.50`) 위에 얹는다. 별점 5점 `+0.50` / 4점 `+0.20` / 3점 `0` 초안 |
...
### 4.11 만남 성립 판정 정리 — 구현 완료 (수동 검증 대기)

2026-09-13 사용자 제보 2건에서 출발했고, 2026-09-14에 정책을 확정하고 구현했습니다.
브랜치는 `fix/wbs-10-b-match-completion-and-report-scope`입니다.

**두 건을 한 브랜치로 묶었습니다.** 따로 하면 "만남이 실제로 있었는가"를 두 곳에서 다르게
정의할 위험이 있습니다. 신고 대상 판정과 완료 판정이 같은 사실(도착 여부)을 근거로 삼습니다.

구현 결과와 판단 근거는 `docs/10_PROGRESS_LOG.md`의
`[10-B 매칭] 만남 성립 판정 정리 — 완료는 시간으로, 신고는 도착으로 (docs/19 4.11)`을 따릅니다.

#### 4.11.1 만남이 성사되지 않은 매칭은 신고 대상에서 제외 — 완료
...
| --- | --- |
| 제외 여부 | 제외한다 |
| 판정 기준 | **도착 여부.** 그룹에 `arrived_at`이 있는 참가자가 한 명도 없으면 신고 불가 |
| 화면 | 4.10과 같이 버튼 비활성화. 안내 문구는 기간 만료와 구분 |
| 오류 코드 | `REPORT_MEETING_NOT_HELD`(409) |

**경과 시간이 아니라 도착 여부로 잡은 이유**가 노쇼입니다. "확정 후 N분 이내 취소" 기준으로
자르면 25분 만에 깨진 건은 아무도 만나지 않았는데 신고 대상이 되고, 반대로 N을 크게 잡으면
노쇼 신고가 막힙니다. 도착 여부로 보면 상대가 오지 않아 취소된 건은 **기다린 쪽이 이미
`ARRIVED`**라 그대로 신고할 수 있습니다. 노쇼 경로는 막히지 않습니다.

이 판단의 전제는 **자유 채팅이 없다**는 것입니다. 만나기 전 상태방에서 발생할 수 있는 피해가
사실상 없으므로, 도착 전에 깨진 매칭을 제외해도 놓치는 신고가 없습니다. 채팅이 생긴다면 이
결정은 다시 봐야 합니다.

판정은 status가 아니라 **`arrived_at IS NOT NULL`**로 합니다. 도착한 뒤에도 status는 완료 시
`COMPLETED`, 그룹 취소 시 `LEFT`로 바뀌지만 `arrived_at`은 남습니다. status로 판정하면 같은
만남이 그룹 상태에 따라 신고 가능해졌다 불가능해졌다 합니다.

#### 4.11.2 완료 판정을 도착에서 만남 종료 시각으로 — 완료

제보는 "상태방이 생기면 매너온도가 오르는 것 같다"였습니다. 조사 결과 **생성만으로 오르는
경로는 없었고**, 대신 결함 두 개가 나왔습니다.

1. **혼자 도착해도 완료·보상됐다.** 완료 조건이 `activeMembers.allMatch(ARRIVED)`라 활성
   구성원이 한 명이어도 통과했습니다.
2. **닫히지 않는 그룹이 있었다.** 3명 중 2명 도착·1명 노쇼면 도착 시점에는 노쇼가 아직
   `JOINED`라 완료되지 않고, 노쇼 배치가 그를 `NO_SHOW`로 바꾸면 남은 둘이 모두 `ARRIVED`가
   되어 노쇼 후보 조회(`JOINED`/`ARRIVAL_TIME_SELECTED`가 있는 그룹만)에 다시 걸리지
   않습니다. 그룹이 `IN_PROGRESS`로 영구히 남고, `existsActiveByMemberId`가 계속 참이라
   **끝까지 참여한 회원의 새 매칭 신청이 영구히 막혔습니다.**
...
| 전원 도착 시 | 완료하지 않는다. 상태방을 유지하고 "전원 도착 · 만남 진행 중"으로 표시 |
| 완료 조건 | 도착자 **2명 이상**(`MINIMUM_ARRIVED_MEMBERS`). 보상은 도착자에게만 |
| 도착자 1명 이하 | `CANCELLED`, 사유 `INSUFFICIENT_ARRIVALS`(`V38`) |
| 도착 마감 | 30분 그대로. 목적이 다르다(노쇼 판정 대 만남 길이) |
| 2인 미동의 그룹 | **둘 다 도착했으면 유지한다.** 한 명이라도 안 왔으면 예전대로 종료 |
| 소급 보정 | 하지 않는다. 필요하면 4.9 PR A의 관리자 수동 조정으로 개별 처리 |

**1시간인 이유는 둘입니다.** 사용자 기준으로 축제 구경에 그 정도가 걸리고, 코드 기준으로는
재매칭 잠금(`MatchCompletionLockPolicy.MATCH_VALIDITY`)이 이미 확정 + 1시간입니다. 두 값이
같아야 **방이 닫히는 순간 잠금도 풀립니다.** 30분으로 잡으면 "만남은 끝났는데 30분 더 신청이
안 되는" 구간이 생깁니다. 두 상수는 함께 움직여야 합니다.

**`MINIMUM_TWO_NOT_ALLOWED` 판정도 도착 여부를 본다.** 예전에는 노쇼로 2명이 남으면 그중 한
명이라도 2인 진행에 동의하지 않았을 때 무조건 종료했는데, 판정 시점인 도착 마감에는 둘 다 이미
현장에 나와 마주 본 상태입니다. "2명은 싫어요"는 **만나기 전**의 의사이고, 거기서 취소해도 만난
사실은 달라지지 않으면서 끝까지 나온 두 사람만 보상을 잃습니다. 한 명이라도 오지 않았다면
만남이 시작되지 않았으므로 사전 의사가 그대로 유효합니다.

**도착 즉시 완료를 없앤 이유**는 그 시점이 만남의 끝이 아니라 시작이기 때문입니다. 예전에는
마지막 도착자가 버튼을 누르는 순간 상태방이 사라지고 보상이 지급됐습니다.

종료 판정은 `MatchMeetingCloseGroupService`가 하고 `MatchMeetingCloseScheduler`(5초 주기)가
돌립니다. 노쇼 배치와 달리 `app.matching.scheduler.enabled`에 겁니다. 노쇼 처리는 페널티라
환경에 따라 끌 수 있지만 그룹을 닫는 것은 수명주기의 일부여서, 꺼지면 참가자가 새 매칭을
신청하지 못한 채 남습니다.

**아직 도착도 노쇼도 아닌 구성원이 남아 있으면 종료를 다음 주기로 미룹니다.** 여기서 노쇼를
대신 찍으면 penalty 적용까지 복제하게 됩니다. 도착 마감(30분)이 만남 종료(1시간)보다 앞서므로
정상 운영에서는 한 주기면 정리됩니다.

#### 4.11.3 도착 인증과 먼저 나가기 — 완료

**도착 인증에 GPS 검증을 넣었습니다.** 예전에는 서버가 좌표를 아예 받지 않고
`arrive(memberId)`만으로 도착을 인정했습니다. 화면에 "도착 인정 반경 150m"가 떠 있었지만
그 값은 하드코딩된 안내 문구였고 검증과 아무 관계가 없었습니다.

체크인(`app.festival.checkin`)과 같은 구조로 맞췄습니다.

| 항목 | 값 |
| --- | --- |
| 반경 | `app.matching.arrival.radius-meters` (기본 `150`) |
| 우회 | `app.matching.arrival.bypass-radius-check` (운영 `false`, local·dev `true`) |
| 환경변수 | `MATCHING_ARRIVAL_RADIUS_METERS`, `MATCHING_ARRIVAL_BYPASS_RADIUS_CHECK` — **둘 다 선택** |
| 저장 | `match_group_members.arrival_distance_meters` — **거리만. 좌표는 저장하지 않는다** |
| 장소 좌표 없는 그룹 | 검증하지 않는다. 사용자 잘못이 아닌데 만남이 무산되면 안 된다 |

**환경변수는 yml 기본값을 덮어쓸 때만 씁니다.** yml이 `${MATCHING_ARRIVAL_RADIUS_METERS:150}`
형태라 `.env`에 없으면 기본값이 그대로 쓰이고, local·dev 프로필은 우회가 이미 `true`라 개발 중에
따로 설정할 것이 없습니다. `.env.example`에는 **주석 처리해서** 넣었습니다. 그 파일을 복사해
운영 환경 설정으로 쓰면 GPS 검증이 꺼진 채로 배포되기 때문입니다.

반경 기본값을 `30`이 아니라 `150`으로 둔 이유는 GPS 오차입니다. 축제장은 인파·천막·건물로
오차가 커서 `30m`면 옆에 서 있는데도 도착이 안 눌리고, 그러면 만남은 성사됐는데 노쇼로 찍힙니다.
값은 설정이므로 운영하며 조정합니다. 화면에 뜨는 반경도 이제 같은 설정값을 씁니다.

**"먼저 갈게요"를 넣었습니다.** 도착 버튼을 누르면 그 뒤로는 빠져나올 방법이 없었습니다.
`MatchCancellationService`가 `ARRIVED`를 거부하고 화면의 취소 버튼도 도착과 함께 사라져서,
상대가 오지 않아도 도착 마감까지 묶였습니다.

**나가는 경로가 만남 성립 여부로 갈립니다.** 처음에는 도착한 사람 모두에게 무패널티 이탈을
주려 했는데, 그러면 **도착 버튼을 눌렀다 나가는 것만으로 취소 페널티를 피할 수 있습니다.**
오고 있는 사람에 대한 책임이 버튼 하나로 사라지므로 경계를 그었습니다.

| 내 상태 | 쓰는 경로 | 페널티 | 매너온도 |
| --- | --- | --- | --- |
| 도착자가 나 혼자 (미성립) | **참여 취소**(`MatchCancellationService`) | 기존 규칙 그대로 — 확정 3분 이내 무패널티, 이후 페널티+쿨타임 | 없음 |
| 도착자 2명 이상 (성립) | **먼저 갈게요**(`MatchLeaveService`) | 없음 | 그대로 받음 |

이를 위해 참여 취소가 `ARRIVED`도 받도록 열었습니다. 예전에는 도착과 동시에 취소 경로가
막혀서 혼자 기다리는 사람이 도착 마감까지 묶였습니다. 이제 **페널티를 감수하면 나갈 수 있고**,
30분을 기다리면 노쇼 배치가 무패널티로 정리합니다. 두 경로는 겹치지 않습니다 — 만남이 성립한
뒤에는 취소가 거절되고 이탈만 남습니다.

**만난 뒤의 이탈에는 페널티가 없습니다.** 그 버튼은 벌을 정하는 자리가 아니라 빠져나올 문이고,
자리가 불편하거나 무서울 때 쓰는 경로에 페널티를 붙이는 것은 안전 설계에 반합니다.

**머문 시간을 재지 않습니다.** 확정 + 1시간은 방을 닫는 행정적 시각이지 그만큼 머물기로 한
약속이 아닙니다. 40분 놀다 헤어진 것과 1시간을 채운 것을 가를 근거가 없으므로, 먼저 갔는지로
보상을 가르지 않습니다. 판정은 하나뿐입니다.

> **만남이 성립했나 (= 도착자가 2명 이상인가)**
> - 성립 → 보상 받고, 확정+1시간까지 재매칭 잠금
> - 미성립 → 보상 없고 잠금도 없음. 즉시 재매칭

잠금은 벌이 아니라 "이번 만남은 이미 했다"는 뜻이고, 끝까지 남은 사람에게도 똑같이 걸립니다.

이 결정에 따라 세 곳이 함께 바뀌었습니다.

| 대상 | 전 | 후 |
| --- | --- | --- |
| 완료·보상 판정 | 종료 시점 `status = 'ARRIVED'` 2명 이상 | **`arrived_at` 2명 이상.** 먼저 나간 사람도 포함 |
| 그룹 유지 판정 | 활성 인원만 보고 1명 이하면 종료 | **이미 만난 방은 종료하지 않는다.** 남은 사람이 혼자여도 만남 시간까지 유지 |
| 재매칭 잠금 | `status = 'COMPLETED'`인 그룹 기준 | **만남이 성립했던 그룹 기준.** 나간 사람이 잠금을 피해 가지 못한다 |

마지막 항목이 없으면 구멍이 생깁니다. 나간 사람은 그룹이 완료되기 전에 빠지므로 완료 참여로도
활성 참여로도 잡히지 않아, 만남을 하고도 곧바로 새 매칭을 신청할 수 있게 됩니다. 끝까지 남은
사람만 잠기는 역차별이 되고 "마음에 안 들면 나가서 바로 다시 뽑기"가 가능해집니다.

**상호 확인("서로 만나셨나요?")은 넣지 않았습니다.** ①GPS 반경 검증이 같은 역할을 합니다 —
같은 좌표 반경 안에서 2명 이상이 도착을 눌렀다는 물리적 증거가 생깁니다. ②4.9에서 후기 대신
완료 보상을 택한 이유가 "상대가 안 써주면 잘 참여한 회원도 못 오른다"였는데, 상호 확인을 보상
조건으로 걸면 그 문제가 돌아옵니다. ③시점을 어디로 잡아도 걸립니다 — 종료 때 물으면 답할
때까지 보상이 막히고, 하루 뒤에 물으면 이미 준 보상을 회수해야 합니다. 반대 방향(안 만났는데
보상이 갔다)은 `NO_SHOW` 신고로 이미 열려 있습니다.

**오고 있는 사람은 어떻게 되나**: 3~4인 방이면 남은 활성 인원으로 방이 유지되므로 나머지는
그대로 만납니다. 2인 방이면 방이 종료되어 오던 사람이 헛걸음하는데, 그 사람은 도착을 누르지
않았으므로 재매칭 잠금이 없어 **즉시 다시 신청**할 수 있고 나간 사람에게는 취소 페널티가
붙습니다. 다만 "방이 없어졌다"를 제때 알려면 알림이 필요합니다(4.11.5).

**남겨두는 한계**: 혼자 도착한 사람이 일찍 나가면 아직 오지 않은 사람들이 노쇼 페널티를
면합니다. 도착 마감(30분) 전에 그룹이 닫혀 노쇼 판정이 돌지 않기 때문입니다. 도착 전 취소에도
있는 기존 동작이고, 고치려면 "이미 끝난 매칭에 30분 뒤 페널티가 붙는" 흐름이 필요합니다.
기다린 사람은 `arrived_at`이 있어 `NO_SHOW` 사유로 **신고할 수 있으므로** 구제 경로는 있습니다.

#### 4.11.4 migration 번호를 두 번 옮겼다 — 저장소에 없는 migration

이 브랜치의 migration은 `V35` → `V36` → `V37` → **`V38`**로 세 번 옮겼습니다. 두 번 다 **저장소에는
없는 migration이 공유 dev DB에 먼저 적용돼 있었기 때문**입니다.

| 번호 | script | dev DB 적용 | 저장소 |
| --- | --- | --- | --- |
| `V35` | `add_content_engagement_count_indexes` | 2026-09-13 22:42 | **나중에 push됨**(해결) |
...
`V35`는 push돼 해결됐습니다. **`V36`·`V37`은 여전히 저장소에 없습니다.** 우리가 `V38`을 쓰는
한 번호는 겹치지 않지만, 저장소에 없는 migration이 중간에 남아 있으면 **dev DB를 쓰는 통합
테스트와 dev 서버 기동이 계속 막힙니다**(`Detected applied migration not resolved locally`).

**`flyway repair`는 쓰지 않았습니다.** repair는 기록된 체크섬을 내 파일 것으로 덮어써서, 남의
migration을 내 파일로 위장시킵니다. 그쪽 스키마 변경이 이력에서 사라집니다.

**재발을 막으려면 규칙이 하나 더 필요합니다** — *저장소에 push하지 않은 migration을 공유 dev
DB에 적용하지 않는다.* 지금 규칙(`docs/08`)은 "번호는 공유 dev DB를 기준으로 정한다"까지만
있어서, 적용하는 쪽을 막지 못합니다. 세 번 반복된 사고입니다.
...
알려야 합니다.** 4.9가 `V33`·`V34`를, 협업자 작업이 `V35`~`V37`을, 4.11이 `V38`을 잡았으므로 4.7은 `V39`부터입니다. **번호는 저장소 파일 목록이 아니라 공유 dev DB의
`flyway_schema_history`를 조회해서 정합니다.**

PR A와 B는 `MannerTemperaturePolicy`를 공유하므로 B를 A 브랜치 위에서 분기했습니다.
**A를 먼저 병합해야 B의 diff가 정상으로 보입니다.** `docs/19` 1절 상태줄과 7절 브랜치 목록은 4.9 쪽에서 갱신합니다.
