...
| 도착자 1명 이하 | `CANCELLED`, 사유 `INSUFFICIENT_ARRIVALS`(`V36`) |
...
#### 4.11.4 병합 전 선행 조건 — 협업자의 `V35`

`V35__add_content_engagement_count_indexes.sql`이 공유 dev DB에 적용돼 있으나 **저장소 어느
브랜치에도 없습니다.** 지금까지는 저장소 마지막 번호가 `V34`라 Flyway가 `V35`를 "미래
migration"으로 보고 넘어갔지만, 이 브랜치의 `V36`이 생기면 중간에 빠진 migration이 되어
검증이 실패합니다(`Detected applied migration not resolved locally: 35`). dev DB를 쓰는 통합
테스트와 **dev 서버 기동이 함께 막힙니다.** 해당 파일을 만든 사람이 push해야 합니다.
...
| 수동 검증 | 대기. dev 배포 후 ①전원 도착 뒤 방 유지와 종료 예정 시각, ②1시간 뒤 완료와 온도 상승, ③노쇼 섞인 그룹의 종료, ④도착 없이 취소된 건의 신고 버튼 비활성화 |
...
알려야 합니다.** 4.9가 `V33`·`V34`를, 아직 push되지 않은 협업자 작업이 `V35`를, 4.11이 `V36`을 잡았으므로
4.7은 `V37`부터입니다. **번호는 저장소 파일 목록이 아니라 공유 dev DB의
