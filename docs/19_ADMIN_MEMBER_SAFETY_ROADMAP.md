...
| 도착자 1명 이하 | `CANCELLED`, 사유 `INSUFFICIENT_ARRIVALS`(`V37`) |
...
이 브랜치의 migration은 `V35` → `V36` → **`V37`**로 두 번 옮겼습니다. 두 번 다 **저장소에는
...
`V35`는 협업자가 push해 해결됐습니다. `V36`은 여전히 저장소에 없는데, 우리가 `V37`을 쓰므로
당장 막히지는 않습니다. 다만 **다음 사람이 `V37`을 잡으면 같은 일이 또 납니다.**
...
알려야 합니다.** 4.9가 `V33`·`V34`를, 협업자 작업이 `V35`·`V36`을, 4.11이 `V37`을 잡았으므로 4.7은 `V38`부터입니다. **번호는 저장소 파일 목록이 아니라 공유 dev DB의
