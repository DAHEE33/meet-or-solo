...
### 4.11 확인·수정 대기 (2026-09-13 사용자 제보)

브라우저로 확인하다 나온 두 건입니다. **둘 다 정책 결정이 먼저이고 그 뒤에 코드를 고칩니다.**

#### 4.11.1 짧게 취소된 매칭도 신고 대상인지

마이페이지 매칭 기록에서 신고할 때, **확정 직후(예: 3분 이내) 취소돼 실제 만남이 없었던 건도
신고할 수 있습니다.**

현재 구현(`MatchReportService.validateReportWindow`)은 이렇습니다.

- `CONFIRMED`·`IN_PROGRESS`는 기간 제한 없이 신고 가능
- `COMPLETED`는 `completedAt`, `CANCELLED`는 `cancelledAt` 기준으로
  `MatchReportWindowPolicy.WINDOW_DAYS`(14일) 안이면 신고 가능
- **취소까지 걸린 시간은 보지 않는다.** 확정 3분 뒤 취소된 건도 14일 동안 신고 대상이다

`docs/19` 4.10에서 "취소 과정에서 생긴 문제도 신고할 수 있어야 한다"고 정했기 때문에 취소 건을
포함한 것 자체는 의도된 동작입니다. 다만 **만남이 성립조차 하지 않은 건까지 포함할지는 정하지
않았습니다.**

결정해야 할 것:

1. 만남 전 단시간 취소 건을 신고 대상에서 뺄지, 그대로 둘지
2. 뺀다면 기준을 무엇으로 할지 — 취소까지의 경과 시간(예: 확정 후 N분 이내)인지, 아니면
   **아무도 도착하지 않았는지**(`match_group_members`에 `ARRIVED`가 없음)인지.
   경과 시간보다 도착 여부가 "만남이 실제로 있었는가"에 더 가깝다
3. 뺀다면 화면에서 버튼을 숨길지, 눌렀을 때 사유를 안내할지. 4.10은 기간이 지난 건을
   **버튼 비활성화**로 처리하고 있으므로 같은 방식이 일관된다

주의: 신고 대상에서 빼면 **노쇼(상대가 안 나와서 취소된 건)를 신고할 길이 함께 막힐 수
있습니다.** 노쇼는 `match_penalty_events`가 따로 처리하지만, 취소 사유에 따라 신고가 필요한
경우가 있는지 함께 봐야 합니다.

#### 4.11.2 만남이 완료되어야 매너온도가 오르도록

**상태방이 만들어진 것만으로 매너온도가 오르는 경우가 있습니다.** 보상은 실제로 만남이
완료됐을 때만 지급되어야 합니다.

현재 구현은 도착 완료에 걸려 있습니다.

- `MatchArrivalService`가 **활성 구성원 전원이 `ARRIVED`**가 되면 `group.complete()` 후
  `MatchCompletedEvent`를 발행하고, `MatchCompletedEventHandler`가 `AFTER_COMMIT`에서
  `MannerTemperatureRewardService.rewardCompletedMembers()`를 부른다
- 즉 설계상으로는 "상태방 생성만으로" 오르지 않는다

그래서 **재현 조건을 먼저 확인해야 합니다.** 코드상 의심되는 경로가 하나 있습니다.

> 완료 판정이 `activeMembers.stream().allMatch(ARRIVED)`다. 상대가 취소·이탈해 **활성
> 구성원이 혼자만 남으면, 그 한 명이 도착을 누르는 순간 그룹이 `COMPLETED`가 되고 보상이
> 지급된다.** 혼자 도착한 것은 만남이 아니다.

확인·결정할 것:

1. 실제로 어떤 흐름에서 온도가 올랐는지 재현 — 구성원 수, 각자의 상태, `match_events`와
   `manner_temperature_events` 행을 함께 본다
2. 위 "혼자 남은 그룹의 단독 완료"가 원인이면 **최소 인원 조건**을 완료 판정에 넣을지
   (예: 활성 구성원 2명 이상 + 전원 도착). `docs/05`의 최소 매칭 인원과 기준을 맞춰야 한다
3. 보상 조건과 그룹 완료 판정을 분리할지 — 그룹은 `COMPLETED`로 두되 보상만 막는 방식도
   가능하다. 완료 판정을 바꾸면 재매칭 잠금(`MatchCompletionLockPolicy`)과 완료 카드 표시까지
   함께 영향을 받으므로 범위가 커진다
...
알려야 합니다.** 4.9가 `V33`·`V34`를 이미 잡았으므로 4.7은 `V35`부터입니다.
