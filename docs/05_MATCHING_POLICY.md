...
```text
MATCHING_SCHEDULER_FIXED_DELAY (기준 10 seconds)
```

조합은 scheduler tick에서만 수행하므로 이 값이 곧 후보가 한 배치에 모이는 시간입니다.
자세한 영향은 `docs/02` "매칭 Scheduler 플래그"에 있습니다.
...
수집 주기를 두면 후보가 모일 **기회**는 생기지만 보장되지는 않습니다. 그 시간에 두 명만
