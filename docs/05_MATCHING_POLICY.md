...
```text
MATCHING_COLLECT_WINDOW (기준 10 seconds)
```

같은 축제의 **첫 유효 대기자가 들어온 시점부터** 이 시간 동안 후보를 모으고, 수집이 끝난 뒤
모인 후보를 한 번에 평가합니다. 자세한 규칙은 아래 "후보 수집 구간"에 있습니다.
...
### 조합 경로는 scheduler 하나뿐이다

3번에 나오듯 후보 조합은 scheduler tick에서만 일어납니다. 매칭 신청 직후 즉시 조합하는
경로는 제거했습니다.

그 경로는 **유효한 조합이 처음 생기는 순간 즉시 소진**시켰습니다. 그래서 대기 후보가 2명을
넘지 못했고, 가능한 조합이 1개뿐이면 조합을 정렬해서 고를 대상이 없으므로 궁합 점수가 순위에
개입할 수 없었습니다. 결과적으로 취향 임베딩이 매칭 결과를 바꾸지 못하고 먼저 신청한
사람끼리 묶였습니다.

### 후보 수집 구간

tick마다 "지금 대기 중인 후보 전체"를 평가하면 **유효한 조합이 처음 생기는 순간 소진**됩니다.
그러면 대기 후보가 2명을 넘지 못하고, 조합이 1개뿐이면 정렬해서 고를 대상이 없으므로 궁합 점수가
순위에 개입할 수 없습니다. 그래서 축제별로 수집 구간을 두고 그 구간이 끝난 뒤에만 평가합니다.

| 규칙 | 내용 |
| --- | --- |
| 시작 | 같은 축제의 **첫 유효 대기자**가 들어온 시점 |
| 종료 | 시작 + `MATCHING_COLLECT_WINDOW`. **한 번 정해지면 바꾸지 않습니다** |
| 연장 | 신규 신청이 들어와도 **연장하지 않습니다** |
| 첫 신청자 이탈 | 취소·매칭·만료되어도 **종료 시각은 그대로**입니다 |
| 종료 후 신청 | 그 구간이 아니라 **다음 구간**에서 평가됩니다 |
| 잔여 후보 | 다음 구간으로 넘어갑니다. 대기 시각과 만료 시각은 초기화하지 않습니다 |
| 전원 이탈 | 그 구간은 종결되고, 다음 신규 신청이 새 구간을 시작합니다 |
| 축제 | 구간은 축제별로 독립입니다 |
| 최소 인원 | **없습니다.** 2명만 있어도 수집이 끝나면 매칭됩니다 |

수집 시간과 scheduler 주기는 **다른 값**입니다. 제안이 실제로 만들어지는 시점은
`수집 종료 + (0 ~ 조합 주기) + 처리 시간`이므로, 조합 주기(`MATCHING_SCHEDULER_MATCHING_FIXED_DELAY`)는
수집 시간보다 짧아야 합니다. 기동 시 검증합니다.

한 구간은 **반드시 한 실행이 통째로 평가**합니다. 여러 실행이 나눠 가지면 같은 구간의 후보가
서로 비교되지 않습니다.

### 궁합 점수가 결과를 바꾸는 조건

점수는 **매칭할지 말지를 판정하지 않습니다.** 최소 궁합 점수 임계값이 없으므로 점수가 낮아도
유효한 조합이면 매칭됩니다. 점수는 **여러 조합 사이의 순위만** 가리고, 그 순위도 인원 수보다
뒤입니다(인원이 많은 조합이 먼저입니다).

따라서 점수가 결과를 바꾸려면 같은 tick에 **유효한 조합이 2개 이상** 있어야 합니다.
2인 매칭 기준으로는 유효 후보가 3명 이상 모여야 합니다.

수집 구간을 두면 후보가 모일 **기회**는 생기지만 보장되지는 않습니다. 그 시간에 두 명만
모이면 여전히 그 둘이 매칭됩니다. 이것은 밀도의 한계가 아니라 **임계값 없이 지금 가능한
조합을 즉시 선택하는 현재 정책의 결과**입니다. 임계값이나 "더 나은 상대를 기다리는" 정책을
두면 후보가 적을 때 선택을 유보할 수 있으며, 둘 다 실사용 점수 분포를 확보한 뒤 판단합니다.
...
매칭이 실패로 종료됐을 때 사용자를 솔로 코스로 유도할지 재매칭을 기다리게 할지는
**서버나 화면이 판단하지 않고 사용자가 고른다.**

- 종료 카드에 `다시 신청하기`와 `솔로 코스 추천 보기`를 동등한 두 버튼으로 나란히 둔다.
- 재신청 가능 시점 안내는 기존 cooldown 표시를 그대로 사용한다. cooldown이 active면
  `다시 신청하기`를 비활성화하고 남은 시간을 카운트다운으로 보여준다.
- `솔로 코스 추천 보기`는 cooldown과 무관하게 항상 활성이다. 기준 축제(`festivalId`)를 모르면
  버튼을 숨긴다.
- 잔여 cooldown이나 체크인 잔여시간을 기준으로 한 임계값 유도 규칙은 두지 않는다.
  cooldown이 이미 "기다릴 이유가 있으면 재신청을 막는" 역할을 하므로 규칙이 중복된다.

종료 카드 자체의 노출 조건은 cooldown 유무가 아니라 **이 세션에서 진행 상태를 실제로 관측한
pool이 종료됐는지**로 판단한다.

- 세션 중 실시간으로 종료를 관측한 pool은 종료 카드를 한 번 보여준다.
- 새 mount에서 발견한 과거 terminal pool은 곧바로 신청 화면으로 돌려보낸다.
  이는 `[10-A 후속 2]`의 "종료 화면 고착" 수정을 유지하기 위한 조건이다.

탐색 만료(비귀책 `EXPIRED`)는 cooldown을 만들지 않으므로 두 버튼이 모두 활성이다.
상세 근거는 `docs/26_MATCH_FAILURE_SOLO_COURSE_LINK_DESIGN.md`를 따른다.
...
재매칭 잠금 기간과 완료 화면 표시 기간은 다른 개념입니다. 잠금은
`confirmed_at + 1시간`이고 이 값만 새 pool 신청을 막습니다. 완료 card 표시는
Frontend 규칙으로, 잠금이 살아 있거나 완료한 날과 서울 기준 같은 날일 때만
보여주고 날이 바뀌면 신청 화면으로 돌아갑니다. 한쪽을 바꿀 때 다른 쪽을 같이
바꾸지 않습니다.
