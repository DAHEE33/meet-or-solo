...
[docs/31](31_WBS10B_HANDOVER.md) 인수인계 3건과 사용자 추가 요청 3건을 합쳐
[docs/32](32_NEXT_WORK_PRIORITY_PLAN.md)로 순서를 정하고 그대로 구현했다. 중복을 걷어내니
실제 항목은 다섯 개였다 — 오류 문구(A), GPS 바이패스(B), 알림함(C/D), 30도 제한(F), Push(E).

### "GPS 바이패스가 안 된다"는 결함이 아니었다

가장 먼저 진단한 항목이고, **코드에는 문제가 없었다.** 체크인·도착 모두 계정 단위 면제
(`members.test_account`)가 제대로 걸려 있었다. 막힌 곳은 그 앞이었다.

> 테스트 계정은 `/admin/members`에서 지정한다 → 그 화면에 들어가려면 관리자 계정이 필요하다 →
> 슈퍼관리자 계정은 `ADMIN_LOCAL_USERNAME`·`ADMIN_LOCAL_PASSWORD`가 **둘 다 있을 때만** 생성된다 →
> `.env`에도 `.env.example`에도 그 항목이 없었다.

관리자 로컬 로그인(`docs/30-2`) 코드는 이미 있었는데, **채울 값이 있다는 사실 자체가 어디에도
적혀 있지 않아** "기능이 없다"로 보였다. `.env.example`에 항목과 설명을 넣고,
`scripts/diagnose-gps-bypass.sql`과 `docs/30` 4.5에 3단계 진단(응답 코드 → 플래그 → 서버 로그)을
남겼다.

**교훈: 환경변수가 없으면 조용히 꺼지는 기능은, 그 변수를 example에 적어야 존재하는 기능이 된다.**

### 체크인 오류 문구 — 순서가 문제였다

"반경 밖인데 정확도가 낮다고 뜬다"는 제보의 원인은 검증 순서였다. 정확도를 거리보다 먼저
보는데, 실내·PC 측위는 정확도가 수백 m로 나오는 일이 흔하다. 그래서 **반경 밖에 있는 사람은
사실상 항상 "정확도가 낮아요"만 보고 같은 자리에서 다시 눌렀다.**

거리를 먼저 보게 바꾸고, 반경 밖 거절에는 떨어진 거리를 담았다("축제에서 약 1.2km 떨어져
있어요"). 도착 인증도 같은 방식이다.

고치면서 더 큰 구멍을 찾았다. **도착 확인 대화상자에는 오류를 보여줄 자리가 아예 없었다.**
반경 밖으로 거절돼도 화면에 아무 변화가 없었고, 다른 자리에서는 "도착 예정 시간을 저장하지
못했어요"라는 엉뚱한 문구가 떴다 — `actionError` 한 자리를 네 개 버튼이 함께 썼기 때문이다.
`actionErrorSource`로 어느 버튼이 실패했는지 구분했다.

### 알림 2단계 — localStorage에서 서버 알림함으로

1단계는 기기 안에 최근 20건만 남겨서 로그인해도 못 본 알림이 복원되지 않았다. `notifications`
테이블(`V39`)과 조회·읽음 API를 만들고 프론트 store를 서버 목록과 합쳤다.

확정한 값은 넷이다.

| 항목 | 결정 |
| --- | --- |
| 보관 | 30일 **과** 100건 둘 다. 먼저 걸리는 쪽이 지운다 |
| 저장 대상 | 종결·행동 필요 7종만. 도착·예정 시간 같은 중간 상태는 저장하지 않는다 |
| 읽음 | 목록을 열면 전체 읽음(1단계와 동일) |
| 행위자 | `actorMemberId`를 이벤트에 실어 **본인에게는 남기지 않는다** |

마지막 항목이 `docs/31` 5절의 "알림 자기 반향"을 닫는다. 저장은 WebSocket 발송 **뒤에**, 별도
transaction에서 한다 — 저장 실패가 실시간 알림을 막으면 안 된다(`MannerTemperatureRewardService`와
같은 이유).

### 30도 매칭 제한 — 무엇을 보여줄지가 핵심이었다

`MatchPoolEntryService.enter()`에 온도 조건을 넣는 것 자체는 몇 줄이다. 정작 정할 것은
"제한에 걸린 사람에게 무엇을 보여줄 것인가"였고, 이렇게 정했다.

- **신고를 드러내지 않는다.** 낮은 온도는 곧 "신고를 받았다"라서 사유를 적으면 같은 만남에
  있던 사람 중 누가 신고했는지 좁힐 수 있다(`docs/19` 4.8)
- **카운트다운을 두지 않는다.** 회복은 만남 완료와 시간 경과 두 경로에 달려 있어 확정된 해제
  시각이 없다. 쿨타임처럼 남은 시간을 보여주면 지킬 수 없는 약속이 된다
- **회복 방법과 기준 온도는 알려준다.** 본인 온도는 이미 본인에게 보이는 값이다
- 쿨타임과 함께 걸리면 **온도 제한을 먼저** 보여준다. 2분 카운트다운이 끝난 뒤 다시 막히면 더 나쁘다

자동 제한은 회원 status를 바꾸지 않는다(`docs/19` 4.3). 신고 4건이라야 걸리므로 관리자 안전
알림(3건)이 먼저 뜬다 — 차감량을 `2.00`으로 낮춘 이유가 이것이고, 그 순서를
`MannerTemperatureMatchingLimitTest`가 고정한다.

### Web Push — service worker를 우리 파일로 바꿨다

`generateSW`는 service worker를 생성만 해서 `push` 핸들러를 넣을 자리가 없었다.
`injectManifest`로 바꾸고 `src/sw.ts`를 실제 service worker로 삼았다. **기존 navigation
fallback과 `/api`·`/ws` denylist를 그대로 옮겼다** — 그것을 빠뜨리면 OAuth 로그인이 조용히
실패한다(이전 `vite.config.ts` 주석에 남아 있던 사고다).

payload에는 `{reason, occurredAt}`만 싣고 문구는 service worker가 프론트 매핑으로 만든다.
push로 보내는 사유는 `MATCH_PROPOSED`·`MATCH_CONFIRMED` 둘뿐이다 — 잠금 화면까지 올라오는
경로라 "지금 손을 써야 하는 것"만 보낸다. 권한은 **첫 매칭 신청 직전**에 한 번 묻는다.

VAPID 키가 없으면 push만 꺼지고 WebSocket 알림과 알림함은 그대로 동작한다.

### 검증

- backend: **730건 중 35개 클래스 실패. 전부 작업 PC에 Docker가 없어서다**(Testcontainers).
  로직 실패 0건. Docker가 있는 환경에서 한 번 더 돌려야 한다
- frontend: `npx vitest run` 84 files / 792 tests 통과, `npx tsc -b` 통과,
  `npm run build` 통과(injectManifest로 `dist/sw.js` 생성 확인 — precache 7건)
- 새 endpoint 3개는 `SuspendedActivityPolicyCoverageTest`가 분류를 요구해
  `SuspendedActivityPolicy`의 **허용** 목록에 넣었다. 본인 기기 설정이고, 구독 해지를 막으면
  알림을 끄고 싶어도 못 끄게 된다

### migration 번호

저장소 마지막이 `V38`이라 `V39`(알림함)·`V40`(push 구독)으로 잡았다. **적용 전에 공유 dev DB의
`flyway_schema_history`를 반드시 조회한다.** 같은 사고가 이미 세 번 났다.
...
`fix/wbs-10-b-match-completion-and-report-scope`에 이어서 고쳤다. **알림 브랜치를 이 브랜치로
병합했다** — 오늘 저녁 로컬 테스트에서 매칭과 알림을 한 번에 밟으려면 브랜치 하나여야 한다.

### 제보 ① 3명 희망 2명은 영원히 매칭되지 않는다

두 계정이 같은 축제에 체크인하고 **3명 희망 + "2명이어도 괜찮아요"**로 신청했는데 매칭되지
않았다. 2명 희망으로 바꾸자 즉시 매칭됐다.

원인은 `MatchGroupComposer`다.

```java
GroupKey key = new GroupKey(candidate.festivalId(), candidate.preferredGroupSize());
```

후보를 **`(축제, 희망 인원)`으로 버킷을 나누고 그 인원짜리 조합만** 만든다. 3명 희망 버킷에
2명뿐이면 조합이 하나도 생기지 않는다. 그리고 **`allowMinimumTwo`를 조합 단계에서 아예 읽지
않는다.** `MatchingCandidate`에 값은 실려 오는데 쓰이지 않았다.

덜 드러난 문제가 하나 더 있었다. **희망 인원이 다르면 서로 절대 매칭되지 않는다.** 2명을 고른
사람과 3명을 고른 사람은 같은 축제에 있어도 버킷이 달라 영원히 만나지 못한다.

**이미 한 번 겪은 문제다.** 9/10 커밋(`4c74bb5`)에서 같은 지적이 나왔지만 그때는 **문구만
고쳤다** — "2명만 모여도 매칭된다로 읽히지 않게". 동작을 명세에 맞춘 것이라 틀린 판단은
아니었지만, 사용자 기대와 어긋난 명세는 그대로 남았고 같은 제보가 반복됐다.

### 확정 — 희망 인원을 상한으로 본다

같은 축제 후보를 한데 모아 **큰 조합부터** 시도한다. 조합이 유효하려면 둘 다 만족해야 한다.

1. 조합 크기가 구성원 **각자의 희망 인원 이하** — 원하지 않은 큰 그룹에 넣지 않는다
2. 자기 희망보다 작은 조합에 들어가는 사람은 **`allowMinimumTwo`에 동의**했어야 한다

큰 조합을 먼저 고르므로 3명이 모여 있으면 2명으로 쪼개지 않는다. 다만 같은 tick에 두 명뿐이면
바로 2명으로 묶인다. **"조금 더 기다렸다가 3번째를 받자"는 조건은 넣지 않았다.** 검색 창이
60초뿐이라 기다림의 실익보다 못 만나고 끝나는 손해가 크고, "왜 N초인가"를 정당화할 근거도
없다. `targetGroupSize`는 조합 크기에서 나오므로 2인 그룹은 2인 제안으로 나간다.

화면 문구도 맞췄다. "2명만 남아도 계속 진행 / 인원이 모인 뒤 일부가 빠져도 2명이면 시작해요"
→ **"인원이 적어도 진행 / 희망 인원이 안 모이면 2명이라도 매칭하고, 도중에 빠져도 계속해요"**.
이제 이 토글은 세 자리에서 쓰인다 — ①그룹 구성 ②제안 중 이탈 ③확정 후 감소.

### 제보 ② 재신청에서 버튼이 눌리는데 아무 반응이 없다

쿨타임이 지난 뒤 "다시 매칭하기"로 폼에 들어가 "자동 매칭 신청"을 눌렀는데 **비활성이 아니라
눌리는데도 아무 일이 없었다.** 새로고침하면 정상 동작했다.
...
pool entry가 거절되면 화면 상태를 바꾸지 않고 error만 담는다(`stateAfterPoolEntryFailure`).
그 설계는 "폼을 그대로 두고 다시 누를 수 있게" 하려는 것이라 맞다. 문제는 **그 사유를 재신청
흐름(`retryableTerminal`)에서만 띄운 것**이다. 최초 신청 화면에서 실패하면 화면이 아무 말도
하지 않아 정확히 "먹통"으로 보인다. 흐름을 가리지 않고 띄우도록 고쳤다.

새로고침이 해결책처럼 보였던 것은 mount가 error를 비우기 때문이고, 그 사이 서버 조건(쿨타임
등)이 풀려 다음 클릭이 성공했기 때문이다.

**함께 확인한 것**: 매칭 탐색 취소에는 쿨타임이 붙는다(`MatchPoolCancellationService`).
체크인 1회 안에서 1회 20초, 2회 1분, 3회 5분(+`penalty_score 1`), 4회부터 10분이다. 취소 카드는
카운트다운을 보여주지만 폼에서는 사유가 보이지 않았다. 이제 보인다.

### 검증

- backend: 1043건 중 24건 실패. **22건은 협업자 미push `V35`**(3건이 검증 실패, 19건은 그
  컨텍스트가 죽어 연쇄로 건너뜀), 2건은 기존 baseline. **매칭 쪽 실패는 0건.**
- frontend: `npx vitest run` 75 files / 713 tests 통과, `npx tsc -b` 통과.
- composer 테스트를 새 계약으로 다시 썼다 — 희망 인원이 달라도 작은 쪽에 맞춰 묶기, 희망보다
  큰 그룹 금지, 전원 동의 시 축소, 한 명이라도 거부하면 축소 금지, 큰 조합 우선.

### 로컬 재검증 대기

`docs/30`의 A(신청~확정)를 **3명 희망 + 인원 축소 동의**로 두 계정이 밟으면 ①이 풀렸는지
바로 확인된다. 버튼 무반응은 신청이 거절될 때 **사유 한 줄이 뜨는지**로 확인한다.
...
- frontend: `npx vitest run` 74 files / 705 tests 통과(신규 13건), `npx tsc -b`,
  `npx vite build` 통과.
- **흐름 하네스를 따로 뒀다**(`notificationFlow.harness.test.tsx`). 조각 테스트가 허브·store·문구
  매핑을 하나씩 고정한다면, 하네스는 **서버 이벤트 → 허브 → store → 화면에 보이는 것**이
  순서대로 이어지는지 본다. 매칭 한 판(제안→확정→도착→전원도착→완료)을 흘려보내 배너 2건과
  토스트 3건이 나뉘는지, 종 뱃지가 5로 뜨는지, 상태방에 있을 때 중복되지 않는지, 재연결로 같은
  알림이 와도 한 번만 쌓이는지, 화면과 함께 구독해도 소켓이 하나인지를 확인한다.
  jsdom이 없어 DOM 이벤트 대신 **서버 렌더 마크업**으로 종을 확인했고, 클릭 이후 동작은
  `docs/30` 수동 검증이 맡는다.
...
## [10-B 매칭] 도착 GPS 인증과 먼저 나가기 (docs/19 4.11.3)

상태: Backend/Frontend 구현·자동 테스트 완료. dev 브라우저 수동 검증 대기

같은 브랜치(`fix/wbs-10-b-match-completion-and-report-scope`)의 후속 커밋이다. 앞선 작업에서
완료 판정을 시간으로 옮긴 뒤, 사용자 확인 과정에서 두 가지가 더 나왔다.

### 도착이 검증되지 않고 있었다

`arrive(memberId)`가 좌표를 아예 받지 않았다. **버튼만 누르면 도착이었다.** 화면에는 "도착 인정
반경은 장소 핀 기준 150m예요"가 떠 있었는데, 그 값은 `MatchGroupMeetingPointResponse`에
하드코딩된 상수였고 서버 판정과 아무 관계가 없었다. 안내만 있고 검증이 없는 상태였다.

체크인에 이미 같은 장치가 있어서 그대로 맞췄다. `FestivalCheckinService`는 좌표를 받아
`GeoDistanceCalculator`로 거리를 재고, `app.festival.checkin.bypass-radius-check`로 local·dev에서만
검증을 건너뛴다.

| 항목 | 값 |
| --- | --- |
| 반경 | `app.matching.arrival.radius-meters` (기본 `150`) |
| 우회 | `app.matching.arrival.bypass-radius-check` (운영 `false`, local·dev `true`) |
| 저장 | `arrival_distance_meters` — 거리만. **좌표는 저장하지 않는다** |

**환경변수와 yml의 관계를 확인해 뒀다.** yml이 `${MATCHING_ARRIVAL_RADIUS_METERS:150}` 형태라
환경변수는 **기본값을 덮어쓸 때만** 쓴다. local·dev 프로필은 우회가 이미 `true`이므로 개발 중에
`.env`에 넣을 것이 없다. `.env.example`에는 **주석 처리해서** 넣었다 — 그 파일을 복사해 운영
설정으로 쓰면 GPS 검증이 꺼진 채 배포되기 때문이다. `infra/env/.env.dev.example`에는 dev가
테스트 환경임을 드러내려고 값을 그대로 적었다.

**반경 기본값은 사용자가 처음 말한 30m가 아니라 150m로 정했다.** GPS 오차 때문이다. 축제장은
인파·천막·건물로 오차가 커서 30m면 바로 옆에 서 있는데도 도착이 안 눌리고, 그러면 만남은
성사됐는데 노쇼로 찍힌다. 값 자체가 설정이므로 운영하며 조정한다.

좌표를 저장하지 않는 것은 개인정보처리방침과 맞춘 것이다. 체크인이 `distance_meters`만 남기는
것과 같은 원칙이고, 방침 원문에도 그렇게 적혀 있다.

**장소 좌표가 없는 그룹은 검증하지 않는다.** 좌표는 그룹 확정 시 복사되는데 이 기능 이전에
만들어진 그룹이나 좌표 없는 장소가 있을 수 있다. 거기서 도착을 막으면 사용자 잘못이 아닌데
만남이 무산된다.

### 도착하면 빠져나올 방법이 없었다

`MatchCancellationService`는 취소하는 사람의 상태가 `JOINED`/`ARRIVAL_TIME_SELECTED`일 때만
받는다. 화면의 "못 갈 것 같아요"도 같은 조건이라 **도착 버튼을 누르는 순간 사라진다.** 게다가
도착 마감 30분이 지나면 취소 자체가 막힌다. 즉 혼자 도착해 상대를 기다리는 사람이 30분을
꼼짝없이 묶여 있었다.

그래서 `MatchLeaveService`(먼저 갈게요)를 넣었다. **페널티가 없다.** 이 버튼은 벌을 정하는
자리가 아니라 빠져나올 문이고, 쓰이는 상황이 ①혼자 기다리다 포기 ②자리가 불편하거나 무서움
둘이다. 후자에 페널티를 붙이는 것은 안전 설계에 반한다.

### 그런데 그대로 두면 페널티를 피하는 샛길이 된다

사용자 확인에서 나왔다.

> 만났는지도 모르고, 도착하고 그냥 먼저갈게요 하면 나머지 오던 사람들은 어떡해?

맞는 지적이고, 더 큰 문제가 뒤에 있었다. **도착 전에 나가면 취소 페널티(확정 3분 이후)가
붙는데 도착 버튼을 누른 뒤 나가면 무패널티**였다. 요령을 아는 사용자는 도착을 눌렀다 나가는
것으로 책임을 지우고, 오고 있던 사람은 2인 방이면 헛걸음한다.

새 규칙을 만들지 않고 **이미 있는 두 경로를 만남 성립 여부로 갈랐다.**

| 내 상태 | 쓰는 경로 | 페널티 | 매너온도 |
| --- | --- | --- | --- |
| 도착자가 나 혼자(미성립) | 참여 취소 | 기존 규칙 그대로 — 3분 이내 무패널티, 이후 페널티+쿨타임 | 없음 |
| 도착자 2명 이상(성립) | 먼저 갈게요 | 없음 | 그대로 받음 |

그러려면 참여 취소가 `ARRIVED`도 받아야 한다. 예전에는 도착과 동시에 취소가 막혀서 혼자
기다리는 사람이 도착 마감까지 묶였는데, 그게 애초에 이 작업이 시작된 불만이었다. 이제
**페널티를 감수하면 나갈 수 있고**, 30분을 기다리면 노쇼 배치가 무패널티로 정리한다. 사용자가
처음에 한 말이 정확히 이것이었다 — "패널티를 받아서라도 매칭 도착했어요 했는데 취소".

두 경로는 겹치지 않는다. 만남이 성립한 뒤에는 취소가 거절되고(`MATCHING_CONFLICT`) 이탈만,
성립 전에는 이탈이 거절되고(`MATCHING_LEAVE_NOT_ALLOWED`) 취소만 남는다. 판정은 양쪽 다
`MatchGroupContinuationPolicy.meetingHeld()` 하나를 쓴다.

**오고 있던 사람은** 3~4인 방이면 남은 활성 인원으로 방이 유지되어 그대로 만난다. 2인 방이면
방이 종료되어 헛걸음하는데, 그 사람은 도착을 누르지 않았으므로 재매칭 잠금이 없어 즉시 다시
신청할 수 있고 나간 사람에게는 취소 페널티가 붙는다. 다만 "방이 없어졌다"를 제때 알려면
알림이 필요하다.

### 설계가 한 번 뒤집혔다 — "중도 이탈"을 버렸다

처음에는 이탈을 둘로 갈라 **중도 이탈에 재매칭 잠금을 페널티로 걸려고** 했다. 사용자가 그
전제를 뒤집었다.

> 2명 방에서 둘 다 도착해서 먼저 갈 일이.. 그건 그냥 즐기다 간 거 아니야?
> 꼭 1시간 즐길 필요도 없고 이게 좀 애매하다

맞는 지적이다. **확정 + 1시간은 방을 닫는 행정적 시각이지 그만큼 머물기로 한 약속이 아니다.**
40분 놀다 헤어진 것과 1시간을 채운 것을 앱이 가를 근거가 없고, 3명 방에서 "먼저 갔다"의 기준을
10분으로 할지 30분으로 할지 댈 근거도 없다.

그래서 시간을 재는 설계를 통째로 버리고 판정을 하나로 줄였다.

> **만남이 성립했나 (= 도착자가 2명 이상인가)**
> - 성립 → 보상 받고, 확정+1시간까지 재매칭 잠금
> - 미성립 → 보상 없고 잠금도 없음. 즉시 재매칭

먼저 가든 끝까지 있든 같다. 잠금은 벌이 아니라 **"이번 만남은 이미 했다"**는 뜻이고 끝까지 남은
사람에게도 똑같이 걸리므로 공평하다. 페널티라는 개념도, 나가기 전 경고 팝업도 필요 없어졌다.

### 그 결정이 세 곳을 함께 바꿨다

| 대상 | 전 | 후 |
| --- | --- | --- |
| 완료·보상 판정 | 종료 시점 `status = 'ARRIVED'` 2명 이상 | **`arrived_at` 2명 이상.** 먼저 나간 사람도 포함 |
| 그룹 유지 판정 | 활성 인원만 보고 1명 이하면 종료 | **이미 만난 방은 종료하지 않는다** |
| 재매칭 잠금 | `status = 'COMPLETED'`인 그룹 기준 | **만남이 성립했던 그룹 기준** |

**마지막이 없으면 구멍이 생긴다.** 나간 사람은 그룹이 완료되기 전에 빠지므로 완료 참여로도
활성 참여로도 잡히지 않는다. 그대로 두면 만남을 하고도 곧바로 새 매칭을 신청할 수 있어, 끝까지
남은 사람만 잠기는 역차별이 되고 "마음에 안 들면 나가서 바로 다시 뽑기"가 가능해진다. 그래서
`findLatestCompletedByMemberId`를 `findLatestHeldMeetingByMemberId`로 바꿨다.

**두 번째 변경은 조회 가드도 건드렸다.** `MatchGroupQueryService`가 활성 참여자 2명 미만을
정합성 오류로 보고 있었다. 이제 상대가 먼저 나가 혼자 남는 것이 정상 상태라서, 그대로 두면
남은 사람이 상태방을 열지도 못한다. `participants.isEmpty()`로 완화했다.

`MatchGroupContinuationPolicy`는 인자가 활성 구성원에서 **전체 구성원**으로 바뀌었다. 이미
만났는지 알려면 빠져나간 사람의 `arrived_at`까지 봐야 하기 때문이다. 직전 커밋에서 넣은
"둘 다 도착했으면 2인 미동의여도 유지"도 이 조건에 흡수돼 사라졌다.

### 상호 확인은 넣지 않았다

"서로 만나셨나요?"를 묻는 안을 검토하고 보류했다.

- **GPS 반경 검증이 같은 역할을 한다.** 같은 좌표 반경 안에서 2명 이상이 도착을 눌렀다는 물리적
  증거가 생긴다. 좌표를 안 받던 때라면 확인이 필요했겠지만 이제 근거가 있다.
- **`docs/19` 4.9에서 반대 방향으로 이미 결정했다.** 후기 대신 완료 보상을 먼저 넣은 이유가
  "상대가 안 써주면 잘 참여한 회원도 못 오른다"였다. 상호 확인을 보상 조건으로 걸면 그 문제가
  그대로 돌아온다.
- **시점을 어디로 잡아도 걸린다.** 종료 시점에 물으면 답할 때까지 보상이 막히고, 하루 뒤에
  물으면 이미 지급한 보상을 회수해야 한다.
- 반대 방향(안 만났는데 보상이 갔다)은 `NO_SHOW` 신고로 이미 열려 있다.

### 어뷰징 — 지금은 두고 본다

도착에 좌표 검증이 붙었으므로 "둘이 짜고 도착만 누르고 바로 나가서 온도를 먹는" 경로는 현장에
실제로 가야 성립한다. ①같은 축제에 둘 다 GPS 체크인 ②매칭으로 서로 만남 ③1시간에 한 번이라
수지가 맞지 않는다. 문제가 보이면 그때 다룬다.

### 남겨두는 한계 — 노쇼 페널티 면제

혼자 도착한 사람이 일찍 나가면 아직 오지 않은 사람들이 노쇼 페널티를 면한다. 도착 마감(30분)
전에 그룹이 닫혀 노쇼 판정이 돌지 않기 때문이다. 도착 전 취소에도 있는 기존 동작이고, 고치려면
"이미 끝난 매칭에 30분 뒤 페널티가 붙는" 흐름이 필요하다. 대안인 "나갈 때 미도착자를 즉시 노쇼
처리"는 아직 20분 남았는데 오는 중인 사람을 노쇼로 만든다.

**구제 경로는 있다.** 기다린 사람은 `arrived_at`이 있으므로 `NO_SHOW` 사유로 신고할 수 있고,
관리자가 판단해 제재한다. 자동 페널티만 비는 것이지 방치되는 것이 아니다.

### migration

`V36`에 이어 붙였다. **아직 어느 DB에도 적용되지 않은 migration이라 수정이 허용된다**(적용된
migration 수정 금지 규칙은 이미 적용된 것에 대한 것이다). 새 번호를 잡으면 협업자의 미push
`V35` 문제를 한 번 더 건드리게 되므로 번호를 늘리지 않았다.
...
- backend: 매칭·신고·매너온도 관련 테스트 전체 통과. 신규는 도착 반경 밖 거절, 거리만 저장,
  장소 좌표 없으면 검증 생략, 혼자 도착 후 나가면 그룹 종료, 만남 성립 후 나가도 그룹 유지와
  양쪽 보상, 도착 전 이탈 거절.
- frontend: `npx vitest run` 73 files / 694 tests 통과, `npx tsc -b` 통과.
- 수동 검증 대기: ①`bypass-radius-check`를 끄고 반경 밖에서 도착 시 거절, ②반경 안 도착 시
  `arrival_distance_meters` 기록, ③혼자 도착 후 "먼저 갈게요" → 즉시 재신청 가능,
  ④둘 다 도착 후 한 명 나가기 → 방 유지, 1시간 뒤 둘 다 온도 상승, 둘 다 재매칭 잠금.
...
| 도착 마감 | 30분 그대로 |
| 2인 미동의 그룹 | 둘 다 도착했으면 유지, 한 명이라도 안 왔으면 종료 |
...
**`MatchGroupContinuationPolicy`도 같은 기준으로 고쳤다.** 노쇼로 2명이 남았을 때 그중 한
명이라도 `allowMinimumTwo = false`면 무조건 종료했는데, 판정 시점(도착 마감)에 둘 다 `ARRIVED`면
유지한다. 사용자 결정이다. "2명은 싫어요"는 만나기 전의 의사이고, 이미 현장에서 만난 뒤에
앱이 그룹을 취소하면 만난 사실은 그대로인 채 **끝까지 나온 두 사람만 보상을 잃는다.** 한
명이라도 오지 않았다면 만남이 시작되지 않았으므로 예전 판정을 유지한다.
...
직전(PR #69)은 전문을 보기 전까지 체크박스를 **잠그고** 그 이유를 안내 문구로 달았다.
사용자 확인 결과 그 방식 대신 **누르면 약관이 바로 뜨는 방식**으로 바꾼다. 안내 문구도 뺀다.

### 바뀐 동작

- 체크박스는 더 이상 잠기지 않는다. 전문을 안 본 상태에서 누르면 **체크되는 대신 전문이 뜬다.**
- 모달의 확인 버튼이 `확인했고 동의합니다`로 바뀌고, 누르면 **그때 체크된다.**
- `X`로 닫으면 동의하지 않는다. 다만 이미 열람했으므로 다음 클릭부터는 바로 체크된다.
- `전문 보기` 버튼으로 연 경우는 **열람 의사일 뿐이라 동의로 잇지 않는다.** 버튼 문구도
  `확인했어요`로 다르고, 확인해도 체크되지 않는다.
- 잠김 안내 문구(`ConsentCheckbox`의 `hint`)를 제거했다. 누르면 약관이 뜨므로 "왜 안 눌리지"가
  생기지 않는다.
- 완료 버튼의 "전문을 먼저 확인해 주세요" 분기도 제거했다. 체크박스가 직접 안내하므로
  기존 "동의해 주세요" 한 줄로 충분하다.

### 유지한 것

- **"봤다"의 기준은 모달을 연 시점이다.** 닫는 경로가 둘(확인 버튼, `X`)이라 닫기 기준으로
  잡으면 한 경로가 빠졌을 때 영원히 체크되지 않는 체크박스가 된다.
- 동의 저장 흐름(`agreeAll`), backend, migration 무변경.

### 확인 = 동의로 본 근거

체크박스를 눌러서 연 경우에 한해 확인이 동의로 이어진다. 그 흐름에서는 사용자가 **먼저
"동의합니다"를 누르고** 전문을 본 뒤 확인을 누른 것이므로 동의 의사가 앞선다. 반대로
`전문 보기`는 읽기만 하겠다는 뜻이라 분리했다.

### 검증

- frontend: `npx vitest run` 71 files / 683 tests 통과, `npx tsc -b`, `npx vite build` 통과.
- **클릭 흐름은 임시 jsdom 하네스로 9건 확인**(확인 후 파일·jsdom 삭제, `package.json` 무변경)
  — ①안내 문구 없음, ②체크박스 잠기지 않음, ③첫 클릭에 체크 대신 전문 표시,
  ④`확인했고 동의합니다`로 체크, ⑤`X`로 닫으면 미동의이나 다음 클릭엔 바로 체크,
  ⑥한 번 본 뒤 체크·해제 자유, ⑦처리방침 체크박스는 처리방침만 띄우고 약관은 안 건드림,
  ⑧`전문 보기`로 연 경우 확인해도 미체크, ⑨모달을 거쳐도 입력값 유지.

## [10-B 동의] 전문을 확인하기 전에는 필수 동의를 체크할 수 없게 (docs/19 4.7.1 후속)

상태: Frontend 구현·자동 테스트 완료. 브라우저 수동 검증 대기

직전 작업(PR #66)은 약관·개인정보처리방침 원문을 붙였을 뿐이고 **원문을 보지 않고도 체크할 수
있었다.** `disabled={isSaving}`만 걸려 있어 저장 중에만 잠겼다. 동의의 근거는 "동의 시점에
원문을 확인할 수 있었다"가 아니라 "확인한 뒤 동의했다"에 더 가까우므로 게이트를 넣는다.

### 확정한 것

- **전문을 열기 전에는 체크박스가 잠긴다.** 약관과 처리방침을 각각 따로 판정한다. 하나를
  봤다고 둘 다 풀리지 않는다.
- **잠금만 풀고 체크는 사용자가 직접 누른다.** 모달의 `확인했어요`가 체크까지 대신 켜면
  "확인"을 "동의"로 간주하는 셈이 되어, 나중에 동의 여부를 다툴 때 방어가 약해진다.
- **"봤다"의 기준은 모달을 연 시점이다.** 닫는 시점으로 잡으면 `확인했어요`와 `X` 중 어느
  경로가 빠져도 **영구히 잠긴 체크박스**가 되고, 그건 가입 자체를 막는 버그가 된다.
- **잠긴 이유를 같은 자리에서 알린다.** `ConsentCheckbox`에 `hint`를 추가해 잠김 상태에서는
  "전문을 확인하면 동의할 수 있어요", 풀린 뒤에는 "확인했어요. 동의에 체크해 주세요"를
  보여주고 체크를 마치면 거둔다. 잠긴 체크박스만 두면 왜 눌리지 않는지 알 수 없다.
- **완료 버튼의 안내를 갈랐다.** 전문을 안 본 상태에서 완료를 누르면 "동의해 주세요"가 아니라
  "전문을 먼저 확인해 주세요"가 뜬다. 전자는 체크가 잠긴 상태에서 막다른 안내가 된다.
- AI 동의 2종에는 적용하지 않았다. 선택 동의이고 전문 대신 `자세히` 요약이 전부다.

### 검증

- frontend: `npx vitest run` 71 files / 683 tests 통과, `npx tsc -b`, `npx vite build` 통과.
- **클릭 흐름은 임시 jsdom 하네스로 확인했다**(`npm i -D jsdom --no-save` 후 일회성 테스트,
  확인 뒤 파일과 jsdom 삭제). 7건 전부 통과 — ①초기 두 항목 잠김과 안내 문구, ②잠긴 상태에서
  눌러도 체크되지 않음, ③약관 전문을 열면 약관만 풀리고 처리방침은 잠김 유지, ④닫아도 자동
  체크되지 않음, ⑤풀린 뒤 직접 체크 가능하고 안내가 사라짐, ⑥`X`로 닫아도 잠금 해제,
  ⑦전문 미확인 상태로 완료를 누르면 전문 확인 안내.
  하네스 자체의 오타(연령대 `20s` → `20S`)로 1건이 먼저 실패했고, 프로덕션 결함은 없었다.
- **상시 자동 테스트는 여전히 없다.** 저장소에 jsdom이 없어 클릭 회귀를 막는 방어가 없다.
  정식 도입은 `docs/09` 갱신이 따르는 별도 작업이다.
...
> **⚠️ 아래 4~6절은 9단계(기능 분업 시작 전) 시점의 스냅샷입니다.**
> "아직 하지 않은 것" 목록에는 이후 전부 구현된 항목(Kakao OAuth, JWT, 체크인, 매칭,
> WebSocket, MatchRoomPage, 신고/제재, 테스트 코드 등)이 그대로 남아 있습니다. **현재 남은
> 작업은 이 문서 맨 위의 최신 절들과 `docs/19_ADMIN_MEMBER_SAFETY_ROADMAP.md`를 기준으로
> 판단하세요.** 이 절들은 당시 판단을 남기기 위해 수정하지 않고 보존합니다.
...
- 탭 세션당 1회만 재생한다(`sessionStorage`). OAuth는 같은 탭에서 카카오/네이버를 다녀오고
  ...
  (후속 보완: 로그아웃·탈퇴에서 이 플래그를 지운다. 아래 `[10-UI 후속 3]` 참고.)
...
## [10-UI 후속 3] 로그아웃하면 스플래시를 다시 보여준다

브랜치: `feature/wbs-10-a-festival-course` (축제 코스와 무관한 UI 보완이지만 사용자 결정으로
같은 브랜치에서 이어 작업했다)

### 발단

"로그인 전에는 URL을 칠 때마다 로고 화면이 나오는데, 로그인했다가 로그아웃하면 그 뒤로는
안 나온다"는 관찰이었다. 버그가 아니라 기존 정책의 결과였다. 스플래시는
`sessionStorage`의 `mors:splash-seen`으로 **탭 세션당 1회**만 재생하는데, 로그아웃해도 이
플래그가 남아 있어 같은 탭에서는 다시 뜨지 않는다. 로그인 전에 매번 보였던 것은 그때마다
탭 세션이 새로 시작됐기 때문(새 탭·브라우저 재시작·시크릿)이다.

### 판단

로그아웃은 세션 종료이므로 그 다음 진입을 새 진입으로 취급하는 것이 맞다. 공용 기기라면
다음 사람에게는 실제로 첫 진입이다. `sessionStorage` 정책이나 스킵 경로를 건드리지 않고
플래그만 지우는 방식이라 부작용이 없다.

### 변경

- `splashPolicy.ts`에 `clearSplashSeen()`을 추가했다. `readSplashSeen`/`markSplashSeen`과
  같은 try/catch 래핑이다(시크릿 모드에서는 `sessionStorage` 접근 자체가 throw한다).
- `MyPage`의 `handleLogout`은 `finally`에서 부른다. 서버 호출이 실패해도 화면상으로는
  로그아웃이고 `/login`으로 보내기 때문이다. `handleWithdraw`는 성공 경로에서만 부른다 —
  실패하면 계정이 그대로 남아 세션도 유효하다.

### 알아둘 것

**로그아웃하는 그 순간에 로고가 뜨지는 않는다.** 착지점인 `/login`이 `SKIP_PATHS`라서다.
플래그를 지운 효과는 다음 앱 진입에서 나타난다 — 루트나 딥링크로 다시 들어오거나, 다시
소셜 로그인해서 콜백으로 진입할 때 한 번 재생된다.

### 범위에서 제외

- **세션 만료로 튕기는 경로**(`apiClient`의 `redirectToLoginIfUnauthorized`). 사용자가
  의도한 종료가 아니고, 조회 하나가 401을 받을 때마다 로고가 끼어들면 방해가 된다.

### 테스트

`splashPolicy.test.ts`에 4건 추가(12건 → 16건). 이 저장소의 프론트엔드 테스트에는 DOM이
없어서 `vi.stubGlobal('window', { sessionStorage })`로 `sessionStorage`만 갖춘 최소 window를
심었다(`apiClient.test.ts`와 같은 방식). 기록 후 읽기, 지운 뒤 첫 진입으로 되돌아오는지,
기록이 없을 때 삭제가 조용히 끝나는지, `sessionStorage` 접근이 throw해도 세 함수가 모두
터지지 않는지를 본다.
