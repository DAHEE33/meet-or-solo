...
- `V11__add_member_preference_embeddings.sql` 적용 전에는 PostgreSQL 16 호환 pgvector 이미지로 교체해야 합니다. 이미지 교체 시 기존 data volume을 삭제하지 않고, 재기동 후 `CREATE EXTENSION vector`와 Flyway 이력을 확인합니다.
