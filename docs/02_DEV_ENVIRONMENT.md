...
현재 compose는 `postgres:16-alpine`을 사용하므로 pgvector extension 파일을 포함하지 않습니다. `V11__add_member_preference_embeddings.sql`을 실제 적용하기 전에는 local/dev/prod PostgreSQL 이미지를 PostgreSQL 16 호환 pgvector 이미지로 전환하고 기존 data volume 보존 여부를 확인해야 합니다. 이번 문서·migration 작업에서는 compose 파일과 실행 중인 DB를 변경하지 않습니다.
